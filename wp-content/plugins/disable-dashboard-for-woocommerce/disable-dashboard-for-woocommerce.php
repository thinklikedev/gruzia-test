...
if ( function_exists( 'wcbloat_fs' ) ) {
    ...
    // Hide "Upgrade" and "Change plan" links on Account page in admin - both for multisite and standalone
    function wcbloat_hide_upgrade_links()
    {
        
        if ( function_exists( 'get_current_screen' ) ) {
            $screen = get_current_screen();
            if ( $screen && ($screen->base === 'admin_page_disable-dashboard-for-woocommerce-account' || $screen->base === 'disable-bloat-for-wordpress-woocommerce_page_disable-dashboard-for-woocommerce-account-network') ) {
                wp_enqueue_script( 'wcbloat_hide_upgrade_links_js', plugin_dir_url( __FILE__ ) . 'assets/js/disable-bloat-admin-account.js' );
            }
        }
    
    }
    
    add_action( 'admin_enqueue_scripts', 'wcbloat_hide_upgrade_links' );
